# Database.cs Processor v5 - Line-level brace matching (simpler and more robust)
...
# Line-level brace counting - simpler and robust
function FindMethodEnd {
    param(
        [string[]]$lns,
        [int]$startLine   # line with opening { of method
    )
    
    [int]$depth = 0
    [bool]$inStr = $false  # track multi-line string state
    
    for ([int]$x = $startLine; $x -lt $lns.Length; $x++) {
        [string]$ln = $lns[$x]
        
        # Count quotes (approximate - toggle inStr state for odd count)
        # Also handle \" escape
        [int]$quoteCount = 0
        [bool]$escaped = $false
        for ([int]$q = 0; $q -lt $ln.Length; $q++) {
            [char]$qc = $ln[$q]
            [char]$qprev = if ($q -gt 0) { $ln[$q-1] } else { [char]0 }
            
            # Handle escape sequences
            if ($qc -eq [char]'\\') {
                if (-not $escaped) { $escaped = $true; continue }
                else { $escaped = $false; continue }
            }
            if ($escaped) { $escaped = $false; continue }
            
            if ($qc -eq [char]'"') {
                # check for verbatim string @"... - just increment count regardless
                $quoteCount++
            }
        }
        ...
        # Now count braces only if not in multi-line string
        if (-not $inStr) {
            # Remove everything inside double quotes on this line
            [string]$processed = $ln
            # Remove simple "..." strings (may miss nested, but good approximation for brace counting)
            [string]$clean = ""
            [bool]$inside = $false
            [bool]$wasEscaped = $false
            for ([int]$ci = 0; $ci -lt $processed.Length; $ci++) {
                [char]$cc = $processed[$ci]
                [char]$cp = if ($ci -gt 0) { $processed[$ci-1] } else { [char]0 }
                
                if ($cc -eq [char]'\\' -and -not $wasEscaped) {
                    $wasEscaped = $true
                    $clean += $cc
                    continue
                }
            ...
            # Now count { and } in cleaned line
            for ([int]$bi = 0; $bi -lt $clean.Length; $bi++) {
                [char]$bc = $clean[$bi]
                if ($bc -eq [char]'{') { $depth++ }
                elseif ($bc -eq [char]'}') { $depth-- }
            }
        }
        
        if ($depth -eq 0 -and $x -gt $startLine) {
            return $x  # found method end line
        }
    }
...
while ($i -lt $totalLines) {
    ...
    if ($trimmed -match '^\s*public\s') {
        # Quick check: not field/enum
        if ($trimmed -match ';\s*$' -or $trimmed -match '^\s*public\s+enum\s' -or $trimmed -match '^\s*public\s+class\s' -or $trimmed -match '^\s*public\s+struct\s' -or $trimmed -match '^\s*public\s+interface\s') {
            $output2.Add($line)
            $i++
            continue
        }
        
        # Find ( in next few lines
        [int]$sigStart = $i
        ...
        # Must look like a method - not a field after all, and ( params )
        ...
        # Use LINE-LEVEL brace matching - robust!
        [int]$bodyEnd = FindMethodEnd $lines $bodyStart
        
        # Sanity check - if bodyEnd is more than 500 lines from bodyStart, it's probably wrong
        # but let it go, we'll verify later
        ...
        if ($alreadyWrap) {
            Write-Host "ALREADY: $mname line $($sigStart+1) -> bodyEnd $bodyEnd"
            $script:already++
            for ([int]$oi = $sigStart; $oi -le $bodyEnd; $oi++) {
                $output2.Add($lines[$oi])
            }
        ...
        Write-Host "WRAP: $mname line $($sigStart+1) [$retType] -> lines $($sigStart+1) to $($bodyEnd+1)"
        $script:wrapped++
