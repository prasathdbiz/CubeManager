...
# Read all lines as strong typed array
$lines = [string[]](Get-Content -Path $inputFile)
[int]$totalLines = $lines.Length
Write-Host "Read $totalLines lines"

# Backup
Copy-Item $inputFile ($inputFile + ".bak2") -Force

$output = New-Object System.Collections.Generic.List[string]
$output.AddRange([string[]]$lines)  # We'll use rebuild approach instead: actually rebuild
...
function GetMethName {
    ...
    [string]$bp = $sig.Substring(0, $pi).Trim()
    [string[]]$parts = $bp -split '\s+'
...
while ($i -lt $totalLines) {
    ...
    if ($trimmed -match '^\s*public\s+(?:async\s+)?(?:static\s+)?(?:[a-zA-Z0-9_<>.,\[\]\s?]+)\s+([a-zA-Z0-9_]+)\s*\(') {
        ...
        # Check for existing wrapping - look for try near the start
...
Write-Host ""
Write-Host "Wrapped: $script:wrapped"
Write-Host "Already: $script:already"
